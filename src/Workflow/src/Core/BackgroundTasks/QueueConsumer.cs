...
internal abstract class QueueConsumer : IBackgroundTask
{
    ...
    private async Task Execute()
    {
        ...
        {
            DiagnosticsActivity activity = default;
            try
            {
                var activeCount = 0;
                lock (_activeTasks)
                ...
                if (hasTask)
                {
                    // TODO: changed from ConcurrentHashSet to ConcurrentDictionary
    ...
    private async Task ExecuteItem(string itemId, EventWaitHandle waitHandle, DiagnosticsActivity activity)
    {
        try
        {
